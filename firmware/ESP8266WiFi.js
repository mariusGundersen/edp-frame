...
var netCallbacks = {
  ...
  close(sckt) {
    ...
    if (socks[sckt] == "Wait") socks[sckt] = "WaitClose";
      ...
      if (socks[sckt] < 0 || socks[sckt] == "UDP") socks[sckt] = undefined;
  ...
  recv(sckt, maxLen) {
    if (sockData[sckt]) {
      ...
      if (sockData[sckt].length > maxLen) {
        ...
        r = sockData[sckt];
        sockData[sckt] = "";
        if (socks[sckt] == "DataClose") socks[sckt] = undefined;
      }
      return r;
    }
...
function ipdHandler(line) {
  ...
  var sckt = parms[0];
  if (sockUDP[sckt]) {
    var ip = (parms[2] || "0.0.0.0").split(".").map(function (x) {
      return 0 | x;
    });
    var p = 0 | parms[3]; // port
    sockData[sckt] += String.fromCharCode(
      ip[0],
      ip[1],
      ip[2],
      ip[3],
  ...
  if (len >= parms[1]) {
    ...
    at.getData(parms[1] - len, function (data) {
      sockData[sckt] += data;
    });
    return "";
  }
      ...
      at.cmd("ATE0\r\n", 1000, function cb(d) {
        ...
        if (d == "OK") {
          at.cmd("AT+CIPMUX=1\r\n", 1000, function (d) {
            ...
            if (d != "OK") rej("CIPMUX failed: " + (d ? d : "Timeout"));
            else
              at.cmd("AT+CIPDINFO=1\r\n", 1000, function () {
                // Turn on UDP transmitter info
                res(); // we don't care if this succeeds or not
              });
          });
        } else rej("ATE0 failed: " + (d ? d : "Timeout"));
      });
    });
  },
  reset() {
    return new Promise((res, rej) => {
      at.cmd("\r\nAT+RST\r\n", 10000, function cb(d) {
        ...
        if (d == "ready" || d == "Ready.")
          setTimeout(function () {
            wifiFuncs.init().then(res, rej);
          }, 1000);
        else if (d === undefined) rej("No 'ready' after AT+RST");
        else return cb;
      });
    });
  },
    ...
    return new Promise((res, rej) => {
      at.cmd("AT+CWMODE=1\r\n", 1000, function (cwm) {
        if (cwm != "no change" && cwm != "OK")
          rej("CWMODE failed: " + (cwm ? cwm : "Timeout"));
        else
          at.cmd(
            "AT+CWJAP=" +
              JSON.stringify(ssid) +
            ...
            function cb(d) {
              if (
                ...
                return cb;
              if (d != "OK") rej("WiFi connect failed: " + (d ? d : "Timeout"));
              else res();
            }
          );
      });
...
exports.setup = function (usart) {
  ...
  at.registerLine("WIFI CONNECTED", function () {
    exports.emit("associated");
  });
  at.registerLine("WIFI GOT IP", function () {
    exports.emit("connected");
  });
  at.registerLine("WIFI DISCONNECTED", function () {
    exports.emit("disconnected");
  });
